...
function agruparPedidosMesmoMomento(pedidos) {
    const agrupados = {};
...
function calcularPreco(prato, type, bebida) {
    var _a, _b;
    const precoPF = (_a = precosPratos[prato]) !== null && _a !== void 0 ? _a : 0;
    const precoTipo = type === "comercial" ? precoPF + 5 : precoPF;
    const precoBebida = bebida ? ((_b = precosBebidas[bebida]) !== null && _b !== void 0 ? _b : 0) : 0;
...
async function carregarBalcao() {
    ...
    if (balcaoPedidos)
        balcaoPedidos.innerHTML = "<li>Carregando pedidos...</li>";
    if (historicoPedidos)
        historicoPedidos.innerHTML = "<li>Carregando histórico...</li>";
    try {
        ...
        if (balcaoPedidos)
            balcaoPedidos.innerHTML = `<li style="color:red;">Erro ao carregar pedidos.</li>`;
        if (historicoPedidos)
            historicoPedidos.innerHTML = `<li style="color:red;">Erro ao carregar pedidos.</li>`;
    }
}
function renderPedidosAgrupados(elementId, pedidosAgrupados, podeFinalizar) {
...
window.finalizarPedidoGrupo = async (ids) => {
    ...
    if (!Array.isArray(ids))
        ids = [ids];
    try {
        await Promise.all(ids.map((id) => fetch(`${API}/${id}/finish`, { method: "POST" })));
        carregarBalcao();
    }
    catch (err) {
